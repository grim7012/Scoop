...
//       <div className="si" style={{ position: "fixed", bottom: "40%", right: "40%", transform: "translate(-50%,-50%)", zIndex: 410, width: "min(360px, calc(100vw - 32px))", borderRadius: 10, background: "var(--panel)", border: "1px solid var(--line-hi)", padding: "28px" }}>
  ...
  return (
    <div style={{ height: "100dvh", display: "flex", flexDirection: "column", overflow: "hidden", background: "var(--bg,#0A0908)" }}>
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden", position: "relative" }}>
        ...
        {/* ── CHAT (always full width on mobile) ──────────────── */}
        {/* 
          Layout contract (desktop & mobile):
            main = flex column, fills available height, no overflow
              ├─ scroll-area  (flex:1, overflowY:auto) — empty state OR messages live here
              ├─ chips row    (flexShrink:0) — only when hasMessages
              └─ input bar    (flexShrink:0) — ALWAYS at bottom, never moves
        */}
        <main style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0 }}>

          {/* ── Unified scroll area: empty state OR messages ──── */}
          <div
            className="scr"
            style={{
              flex: 1,
          ...
          >
            {/* Empty state — centered vertically inside the scroll area */}
            {!hasMessages && (
              <div style={{
                flex: 1,
                display: "flex",
                ...
                alignItems: "center",
                justifyContent: "center",
                padding: isMobile ? "24px 16px 60px" : "40px 32px 80px",
              }}>
                {sessions.length > 0 && (
                  <div style={{ width: "100%", maxWidth: 500, marginBottom: 32 }}>
                    <p className="label" style={{ marginBottom: 12 }}>Recent</p>
            ...
            {/* Messages — grow to push content to top, scroll naturally */}
            {hasMessages && (
              <div style={{ padding: isMobile ? "16px 12px 8px" : "24px 24px 8px", maxWidth: 860, width: "100%", margin: "0 auto", alignSelf: "stretch", boxSizing: "border-box" }}>
                {messages.map(m => (
                  <MessageBubble
                    key={m.id}
                    msg={m}
          ...
          {/* Inline chips — outside scroll area, always above input */}
          {hasMessages && (
            <div style={{ flexShrink: 0, padding: isMobile ? "4px 12px 0" : "6px 24px 0", overflowX: "auto", maxWidth: 860, width: "100%", alignSelf: "center", WebkitOverflowScrolling: "touch" }}>
              <div style={{ display: "flex", gap: 6 }}>
                {DEFAULT_CHIPS.slice(0, 4).map(c => (
          ...
          {/* Input — always anchored at bottom, never participates in scroll */}
          <div style={{ flexShrink: 0, padding: isMobile ? "8px 12px 12px" : "10px 24px 18px", maxWidth: 860, width: "100%", margin: "0 auto", alignSelf: "stretch", boxSizing: "border-box" }}>
            <div
              style={{ background: "var(--raised)", border: "1px solid var(--line)", borderRadius: 8, padding: "10px 12px 10px 16px", display: "flex", alignItems: "flex-end", gap: 10, transition: "border-color 0.18s" }}
              onFocus={e => (e.currentTarget.style.borderColor = "var(--gold-border)")}
              onBlur={e => (e.currentTarget.style.borderColor = "var(--line)")}
          ...
          <div
            style={
              isMobile
                ...
                : {
                    /* Desktop: inline flex child — scrolls independently */
                    flexShrink: 0,
                    display: "flex",
                    flexDirection: "column",
                    overflowY: "auto",
